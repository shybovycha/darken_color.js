...
const RGB2HEX = (r: number, g: number, b: number): string =>
  "#" + Math.floor((1 << 24) + (r << 16) + (g << 8) + b).toString(16).slice(1);
...
const NAME2RGB = (name: string): RGBType => {
  ...
  const color: Array<number> = COLORS.get(name) || [0, 0, 0];
...
const parseColorRGB = (color: string): RGBType => {
...
const parseAmount = (amount: string): ColorValueType => {
